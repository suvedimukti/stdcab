...
#' @param strata (character) strata variable; default is NULL, as it does not yield good results with stratification based on class/strata
#' @param breaks (integer) A single number giving the number of bins desired to stratify a numeric stratification variable
#' @param pool   (numeric) A proportion of data used to determine if a particular group is too small and should be pooled into another group. Default is 0.1 \code{\link[rsample]{vfold_cv}}
...
repeated_spatial_cluster_sample <- function(data = data, v = 10, repeats = 1, coords = c("X", "Y"),
  ...
  spatial_clustering_splits_rcv <- function(data, v = 10,
    ...
    if (is.null(strata)) {
      ...
      if (clust_method == "hclust" && !is.null(dist_clust)) {
